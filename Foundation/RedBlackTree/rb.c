...
void * rb_t_find_startsbefore (struct rb_traverser *trav, struct rb_table *tree, void *item)
{
	struct rb_node *p, *q;

	struct rb_node *interestingnode = NULL;
	int interestingheight = 0;
	...
	for( p = tree->rb_root; p != NULL; p = q )
	{
		...
		else 
		{
			q = p->rb_link[1];
			interestingnode		= p;
			interestingheight	= trav->rb_height;
		}
	...
	if( NULL == interestingnode )
	{
		fprintf(stderr,"fasel\n");
		trav->rb_height = 0;
		trav->rb_node = NULL;
		return NULL;
	}
	trav->rb_node	= interestingnode;
	trav->rb_height = interestingheight;
	return interestingnode->rb_data;
}
