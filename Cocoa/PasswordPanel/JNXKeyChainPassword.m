...
- initWithItem:(NSString*)anItemName title:(NSString*)aTitle credentialTypes:(NSArray *)aCredentialTypesArray showPanel:(BOOL)shouldshowpanel;
{
	...
	if( titleString )
	{
		titleString		= [aTitle retain];
	}
	else
	{
		titleString		= [[NSString stringWithFormat:NSLocalizedString(@"Authenticate for %@",@"Default JNXPassword Panel title ( argument is itemName )"),itemName] retain];
	}
...
- (void)dealloc
{
	...
	if( conditionLock)				[conditionLock release];
	if( itemName )					[itemName release];
	if( credentialTypesArray )		[credentialTypesArray release];
	if( titleString )				[titleString release];

	if( credentialsDictionary )		[credentialsDictionary release];
	if( visibleCredentialFields )	[visibleCredentialFields release];
...
- (void)retrieveCredentialsOnMainThread;
{
	...
	if( noErr == (error = SecKeychainFindGenericPassword( NULL, strlen(processname), processname, strlen(itemname), itemname, &keychainitemlength, &keychainitemdata,NULL)) )
	{	
		NSMutableData *keychainItemData = [NSMutableData dataWithBytes:keychainitemdata length:keychainitemlength];
		error = SecKeychainItemFreeContent ( NULL,  keychainitemdata );
	...
	
	{
		...
		else
		{
			...
			for( int i=0 ; i< [credentialTypesArray count]; i++ )
			{
				...
				{
					[credentialTypeButton addItemWithTitle:@"unnamed protocol"];
				}
			}
...
//	[credentialsBox setContentView:[[NSView alloc] initWithFrame:[credentialsBox frame]]];
	...
	
	{
		...
		int			count				= [inputFieldsArray count];
...
- (IBAction)buttonOkHasBeenPressed:(id)sender;
{
	...
	if( (NSOnState == [saveToKeyChainButton state]) && (passwordString = [credentialsDictionary JNXkeyValueStringEncoded]) )
	{
		...
		{
			if( noErr != (error = SecKeychainAddGenericPassword( NULL, strlen(processname), processname, strlen(itemname), itemname, strlen(passwordstring), passwordstring, NULL)) )
			{
		...
		{
			JLog(@"could not access the keychain: %d");
		}
	}
...
- (NSDictionary *)credentials;
{
	...
	return (NSDictionary *)[[credentialsDictionary retain] autorelease];
}
