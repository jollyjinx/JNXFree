...
- init
{
    ...
    singlePopLock		= (NSLock*)[[NSConditionLock alloc] init];
...
- popBeforeDate:(NSDate *)endDate;
{
    ...
    while(1)
    {
        ...
        else
        {
            ...
            [queueLock lock];
            NSAssert2([queueRedBlackTree count] == [queueDictionary count], @"AdvancedDatedQueue popBeforeDate: queueRedBlackTree count:%ld, queueDictionary count:%ld",[queueRedBlackTree count],[queueDictionary count]);
            nowDate = [NSDate date];
...
- (void) push:(id)anObject withDate:(NSDate *)aDate;
{
    ...
    NSAssert2([queueRedBlackTree count] == [queueDictionary count], @"AdvancedDatedQueue push:withDate: queueRedBlackTree count:%ld, queueDictionary count:%ld",[queueRedBlackTree count],[queueDictionary count]);
